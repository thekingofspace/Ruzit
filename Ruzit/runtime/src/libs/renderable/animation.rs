...
pub enum TrackEvent {
    ...
    Stopped,
    Looped,
    Update { link_idx: usize, fire_time: f64 },
}
...
pub struct AnimationTrack {
    ...
    pub played_key: Option<Arc<RegistryKey>>,
    pub stopped_key: Option<Arc<RegistryKey>>,
    pub did_loop_key: Option<Arc<RegistryKey>>,
    pub update_links: Vec<UpdateLink>,
    pub pending_events: Vec<TrackEvent>,
    pub loop_count: u64,
}
...
pub fn tick_track(
    ...
    if track.time > track.duration {
        if track.looped && track.duration > 0.0 {
            while track.time > track.duration {
                track.time -= track.duration;
                track.loop_count = track.loop_count.wrapping_add(1);
                track.pending_events.push(TrackEvent::Looped);
            }
        } else {
            track.time = track.duration;
            track.state = TrackState::Stopped;
            track.pending_events.push(TrackEvent::Stopped);
            eval.finished = true;
        }
    }

    {
        let t_now = track.time as f64;
        let mut pending: Vec<TrackEvent> = Vec::new();
        for (link_idx, link) in track.update_links.iter_mut().enumerate() {
            while t_now >= link.next_fire {
                let fire_time = link.next_fire;
                pending.push(TrackEvent::Update {
                    link_idx,
                    fire_time,
                });
        ...
        track.pending_events.extend(pending);
    }
    let t = track.time;
