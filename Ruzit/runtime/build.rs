...
fn main() {
    ...
    if let (Some(sys_out), Some(target_dir)) = (&sys_out, &target_dir) {
    ...
    if cfg!(target_env = "msvc") {
        println!("cargo:rustc-link-arg-bin=ruzitrun=/DELAYLOAD:steam_api64.dll");
        println!("cargo:rustc-link-arg-bin=ruzitrun=delayimp.lib");
    }
